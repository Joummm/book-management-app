...
export function BookForm({ book }: BookFormProps) {
  ...
  const [rating, setRating] = useState<number | undefined>(book?.rating || undefined)
  const [review, setReview] = useState(book?.review || "")
  const [releaseDate, setReleaseDate] = useState(book?.release_date || "")
  const [startReadingDate, setStartReadingDate] = useState(book?.start_reading_date || "")
  const [finishReadingDate, setFinishReadingDate] = useState(book?.finish_reading_date || "")
  const [pages, setPages] = useState<number | undefined>(book?.pages || undefined)
  const [selectedGenres, setSelectedGenres] = useState<string[]>(book?.genres || [])
  const [customGenre, setCustomGenre] = useState("")
  const [publisher, setPublisher] = useState(book?.publisher || "")
  const [format, setFormat] = useState<"physical" | "digital">(book?.format || "physical")
  const [characters, setCharacters] = useState<string[]>(book?.characters || [])
  const [newCharacter, setNewCharacter] = useState("")
  const [quotes, setQuotes] = useState<string[]>(book?.quotes || [])
  const [newQuote, setNewQuote] = useState("")
  const [wouldReadAgain, setWouldReadAgain] = useState<"yes" | "no" | "maybe" | undefined>(
    book?.would_read_again || undefined,
  )
  const [wouldRecommend, setWouldRecommend] = useState<boolean | undefined>(book?.would_recommend || undefined)
    ...
    if (book) {
      setHasChanges(
        title !== book.title ||
          ...
          pages !== book.pages ||
          JSON.stringify(selectedGenres) !== JSON.stringify(book.genres || []) ||
          publisher !== (book.publisher || "") ||
          format !== book.format ||
          JSON.stringify(characters) !== JSON.stringify(book.characters || []) ||
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (book) {
        ...
        const { error } = await supabase.from("books").update(bookData).eq("id", book.id).eq("user_id", user.id)
  ...
  return (
    <>
      <div className="max-w-4xl mx-auto">
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl">{book ? t.edit : t.addBook}</CardTitle>
            <CardDescription>
              {book ? "Edite as informações do livro" : "Adicione um novo livro à sua coleção"}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="cover">
                  {t.coverImage} <span className="text-muted-foreground text-sm">({t.optional})</span>
                </Label>
                <Input
                  id="cover"
                  value={coverImage}
              ...
              <div className="space-y-2">
                ...
                <RadioGroup value={format} onValueChange={(value) => setFormat(value as "physical" | "digital")}>
              ...
              <div className="space-y-2">
                <Label htmlFor="rating">
                  {t.rating} <span className="text-muted-foreground text-sm">({t.optional})</span>
                </Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="review">
                  {t.review} <span className="text-muted-foreground text-sm">({t.optional})</span>
                </Label>
                <Textarea
                  id="review"
                  value={review}
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <div className="space-y-2">
                  <Label htmlFor="release">
                    {t.releaseDate} <span className="text-muted-foreground text-sm">({t.optional})</span>
                  </Label>
                  <Input
                    id="release"
                    type="date"
                ...
                <div className="space-y-2">
                  <Label htmlFor="start">
                    {t.startReading} <span className="text-muted-foreground text-sm">({t.optional})</span>
                  </Label>
                  <Input
                    id="start"
                    type="date"
                ...
                <div className="space-y-2">
                  <Label htmlFor="finish">
                    {t.finishReading} <span className="text-muted-foreground text-sm">({t.optional})</span>
                  </Label>
                  <Input
                    id="finish"
                    type="date"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="pages">
                    {t.pages} <span className="text-muted-foreground text-sm">({t.optional})</span>
                  </Label>
                  <Input
                    id="pages"
                    type="number"
                ...
                <div className="space-y-2">
                  <Label htmlFor="publisher">
                    {t.publisher} <span className="text-muted-foreground text-sm">({t.optional})</span>
                  </Label>
                  <Input
                    id="publisher"
                    value={publisher}
              ...
              <div className="space-y-2">
                <Label>
                  {t.genres} <span className="text-muted-foreground text-sm">({t.optional})</span>
                </Label>
                ...
                {selectedGenres.filter((g) => !GENRES.includes(g as any)).length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {selectedGenres
                      .filter((g) => !GENRES.includes(g as any))
                      .map((genre) => (
                        <div key={genre} className="flex items-center gap-1 bg-muted px-2 py-1 rounded">
              ...
              <div className="space-y-2">
                <Label>
                  {t.characters} <span className="text-muted-foreground text-sm">({t.optional})</span>
                </Label>
                <div className="flex gap-2">
                  ...
                  />
                  <Button type="button" onClick={addCharacter} variant="secondary">
                    <Plus className="h-4 w-4" />
                  </Button>
                </div>
                {characters.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {characters.map((character) => (
                      <div key={character} className="flex items-center gap-1 bg-muted px-3 py-1 rounded">
              ...
              <div className="space-y-2">
                <Label>
                  {t.quotes} <span className="text-muted-foreground text-sm">({t.optional})</span>
                </Label>
                <div className="flex gap-2">
                  <Textarea
                    ...
                    rows={2}
                  />
                  <Button type="button" onClick={addQuote} variant="secondary" className="self-end">
                    <Plus className="h-4 w-4" />
                  </Button>
                </div>
                {quotes.length > 0 && (
                  <div className="space-y-2 mt-2">
                    {quotes.map((quote, index) => (
                      <div key={index} className="flex items-start gap-2 bg-muted p-3 rounded">
              ...
              <div className="space-y-2">
                <Label>
                  {t.wouldReadAgain} <span className="text-muted-foreground text-sm">({t.optional})</span>
                </Label>
                <RadioGroup value={wouldReadAgain} onValueChange={(value) => setWouldReadAgain(value as any)}>
                  <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="recommend">
                  {t.wouldRecommend} <span className="text-muted-foreground text-sm">({t.optional})</span>
                </Label>
              </div>

              {/* Actions */}
              <div className="flex gap-4 pt-4">
                <Button type="submit" disabled={isLoading || !title || !author} className="flex-1">
                  {isLoading ? "..." : t.save}
                </Button>
      ...
      <AlertDialog open={showExitDialog} onOpenChange={setShowExitDialog}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => router.push("/books")}>Sair sem Salvar</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
